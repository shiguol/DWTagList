...
- (void)display
{
  ...
  for (UIView *subview in [self subviews]) {
    if ([subview isKindOfClass:[DWTagView class]]) {
      ...
      [tagView.button removeTarget:nil
                            action:nil
...
- (void)touchUpInside:(id)sender
{
  ...
  if ([self.tagDelegate respondsToSelector:@selector(selectedTag:tagIndex:)]) {
    ...
    if (jn) {
      ...
      [self.selectedArray setObject:@(0) atIndexedSubscript:tagView.tag];
    } else {
      tagView.isSelected = YES;
      [self.selectedArray setObject:@(1) atIndexedSubscript:tagView.tag];
    }
